...
const textToKeywordObj: MapLike<KeywordSyntaxKind> = {
    ...
    alias: SyntaxKind.AliasKeywordToken,
...
export function lex(sourceCode: SourceCode, errorSink: ErrorSink = new ErrorSink()) {
    ...
    let pushToken = (kind?: SyntaxKind, v: string = value) => {
        tokens.push({
            ...
            fileStart: pos,
            fileEnd: index,
            kind: textToToken.get(v) ?? kind ?? SyntaxKind.Markdown
        });
        pos = index;
        linePos = lineIndex;
        value = '';
    ...
    let _consumeWord = () => {
        _consumeWhile(isUnicodeIdentifierPart);
        let kind: SyntaxKind = textToKeywordObj[value] ?? SyntaxKind.IdentifierToken;
        pushToken(kind);
    }
    let _consumeNumber = () => {
        _consumeWhile(isNumber);
        if (_currentCode() == CharacterCodes.dot) _take();
        _consumeWhile(isNumber);
        if (_currentCode() == CharacterCodes.e) _take();
        _consumeWhile(isNumber);

        pushToken(SyntaxKind.NumberLiteralToken);
    }
    let _consumeWhitespace = () => {
        ...
        while (_currentCode() && isWhitespace(_currentCode()) && indentWhitespaceDepth < tabLength) {
            ...
            lineIndex++;
            loopTrapFunction();
            indentWhitespaceDepth++;
        }
        if (indentWhitespaceDepth == tabLength) {
            pushToken(SyntaxKind.IndentToken);
        }
        else {
            pushToken(SyntaxKind.Whitespace);
        }
    }
    ...
    let _consumeEndStatement = () => {
        _consumeSingle(SyntaxKind.EndStatement);
    }
    let _consumeAttribute = () => {
        _consumeWhile(c => !isNewline(c));
        pushToken(SyntaxKind.Attribute);
    }
    let _consumeSingleLineComment = () => {
        _consumeWhile(c => !isNewline(c));
        pushToken(SyntaxKind.SingleLineCommentToken);
    }
    let _consumeMultiLineComment = () => {
        _consumeWhile(c => !(c == CharacterCodes.asterisk && _nextCode() == CharacterCodes.slash));
        _take(); // *
        _take(); // /
        // took the closing of the multi line comment
        pushToken(SyntaxKind.MultiLineCommentToken);
    }
    ...
    while (index < max) {
        ...
        else if (isStartMultiLineComment(currentChar, _nextCode())) {
            _consumeMultiLineComment();
        }
        else if (isAttributeStart(currentChar)) {
            _consumeAttribute();
        }
        else if (isStringStart(currentChar)) {
            _consumeStringLiteral();
        }
...
function isStartSingleLineComment(c0: number, c1: number) {
    return c0 == CharacterCodes.slash && c1 == CharacterCodes.slash;
}
function isStartMultiLineComment(c0: number, c1: number) {
    return c0 == CharacterCodes.slash && c1 == CharacterCodes.asterisk;
}
function isUnicodeIdentifierStart(c: number) {
    return lookupInUnicodeMap(c, unicodeIdentifierStart);
}
function isUnicodeIdentifierPart(c: number) {
    return lookupInUnicodeMap(c, unicodeIdentifierPart);
}
function isIndent(c) {
    return c == "\t".charCodeAt(0);
}
function isAttributeStart(c: number) {
    return c == CharacterCodes.at;
}
...
function isStringStart(c) {
    return c == CharacterCodes.doubleQuote;
}
